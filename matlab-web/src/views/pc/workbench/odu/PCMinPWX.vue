<template>
  <div class="minpwx-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用抛物线法求解一维函数的极值"/>
    </div>
...
export default {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/odu/minPWX',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='t^2-log(t)-5'
      this.parameters.lOpt=0.5
      this.parameters.rOpt=4
...
<style>
 .minpwx-pc .dialog-container .el-dialog{
  height: 60%;
}
</style>
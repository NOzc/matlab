<template>
  <div class="minjt-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用进退法求解一维函数的极值区间"/>
    </div>
      ...
      <div id="workarea" style="height: 100%;width: 30%;position: absolute;left: 0;">
        <p-c-params-area ref="paramsArea">
          <div>
            ...
            <el-input clearable v-model="parameters.initPoint" :disabled="disabledStatus"></el-input>
            <el-divider>初始步长h0</el-divider>
            <el-input clearable v-model="parameters.initStep" :disabled="disabledStatus"></el-input>
      ...
      <div style="height: 100%;width: 69%;position: absolute;right: 0">
        <p-c-func-img-area ref="funcImgArea">
          <table>
            <tr>
              <td>区间左边界：</td>
              <td><el-input :value="result.minx"/></td>
            </tr>
            <tr>
              <td>区间右边界：</td>
...
export default {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/odu/minJT',
        method: 'get',
        params: {
          func: this.parameters.func,
          initPoint: this.parameters.initPoint,
          initStep: this.parameters.initStep,
        ...
        console.log(res);
        this.result.minx = res.data[0].toFixed(2);
        this.result.maxx = res.data[2].toFixed(2)
        let data = [];
        for (let i = 0;i<res.data[4].length;i++){
          if (i%2===0){
            data.push([res.data[4][i],res.data[4][i+1]])
            i++;
          }
        }
                ...
                data:[
                  {
                    ...
                    name: 'max',
                    coord: [res.data[2],res.data[3]],
                    value: "("+res.data[2].toFixed(2)+","+res.data[3].toFixed(2)+")"
                  }
                ]
              }
            }
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='(t^2-1)^2+(t-1)^2+3'
      this.parameters.initPoint=0
      this.parameters.initStep=0.1
      ...
      this.xyStyle.limit.xliml=-3
      this.xyStyle.limit.xlimr=3
      this.xyStyle.limit.yliml=0
      this.xyStyle.limit.ylimr=8
...
<style>
.minjt-pc .dialog-container .el-dialog{
  height: 60%;
}
</style>
<template>
  <div class="mintri-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用三次插值法求解一维函数的极值"/>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:0,xlimr:2.5,yliml:-4.5,ylimr:0
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/odu/minTri',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='t^4-t^2+t-1'
      this.parameters.lOpt=0
      this.parameters.rOpt=0.1
      ...
      this.xyStyle.limit.xlimr=2.5
      this.xyStyle.limit.yliml=-4.5
      this.xyStyle.limit.ylimr=0
...
<style>
 .mintri-pc .dialog-container .el-dialog{
  height: 60%;
}
</style>
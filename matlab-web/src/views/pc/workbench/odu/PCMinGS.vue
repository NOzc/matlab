<template>
  <div class="mings-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用Goldstein法求解一维函数的极值"/>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:-5,xlimr:5,yliml:0,ylimr:10
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/odu/minGS',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='t^2-2*t+7'
      this.parameters.XMAX=8
      this.parameters.sigma1=0.4
      this.parameters.sigma2=0.75
...
<style>
 .mings-pc .dialog-container .el-dialog{
  height: 60%;
}
</style>
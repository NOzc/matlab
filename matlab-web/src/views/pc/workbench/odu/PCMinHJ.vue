<template>
  <div class="minhj-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用黄金分割法求解一维函数的极值"/>
    </div>
...
import OneDM from "@/components/pc/OneDM";
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:-10,xlimr:10,yliml:0,ylimr:100
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      //向后端发起请求，获取图像
      console.log("黄金分割法获取图像")
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/odu/minHJ',
        method: 'get',
        params: {
          func: this.parameters.func,
          lOpt: this.parameters.lOpt,
          rOpt: this.parameters.rOpt,
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='t^4-t^2-2*t+5'
      this.parameters.lOpt=-10
      this.parameters.rOpt=10
      ...
      this.xyStyle.limit.xliml=-10
      this.xyStyle.limit.xlimr=10
      this.xyStyle.limit.yliml=0
      this.xyStyle.limit.ylimr=100
...
<style>
 .minhj-pc .dialog-container .el-dialog{
  height: 60%;
}
</style>
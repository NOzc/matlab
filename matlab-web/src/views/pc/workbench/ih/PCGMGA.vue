<template>
  <div class="gmga-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用大变异遗传算法求解一维无约束优化问题"/>
    </div>
...
export default {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/ih/GMGA',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      ...
      this.parameters.Pm = 0.03
      this.parameters.alpha=0.6
      this.parameters.Pbm=0.2
...
<style>
.gmga-pc .dialog-container .el-dialog{
  height: 60%;
}/*

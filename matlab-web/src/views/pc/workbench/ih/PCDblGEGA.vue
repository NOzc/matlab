<template>
  <div class="dblgega-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用双切点遗传优化求解一维无约束优化问题"/>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:0,xlimr:1,yliml:0,ylimr:0.7
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/ih/DblGEGA',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='(2*x+1)*exp(-10*x^2+2*x-1)'
      this.parameters.a=0
      this.parameters.b=1
...
<style>
.dblgega-pc .dialog-container .el-dialog{
  height: 60%;
}/*

<template>
  <div class="myga-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用基本遗传算法求解一维无约束优化问题"/>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:0,xlimr:40,yliml:0,ylimr:4500
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      //向后端发起请求，获取图像
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/ih/myGA',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='x^3-60*x^2+900*x+100'
      this.parameters.a=0
      this.parameters.b=30
      this.parameters.NP=50
      this.parameters.NG=100
      ...
      this.xyStyle.limit.xlimr=40
      this.xyStyle.limit.yliml=0
      this.xyStyle.limit.ylimr=4500
...
<style>
.myga-pc .dialog-container .el-dialog{
  height: 60%;
}/*

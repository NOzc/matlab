<template>
  <div class="mmadapga-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area :left="10" @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用多变异位自适应遗传优化求解一维无约束优化问题"/>
    </div>
      ...
      <div id="workarea" style="height: 100%;width: 30%;position: absolute;left: 60px;">
        <p-c-params-area ref="paramsArea">
          <table>
            ...
            <tr>
              <td>杂交常数1：</td>
              <td><el-input clearable v-model="parameters.Pc1" :disabled="disabledStatus"></el-input></td>
            </tr><br/>
            <tr>
              <td>杂交常数2：</td>
              <td><el-input clearable v-model="parameters.Pc2" :disabled="disabledStatus"></el-input></td>
            </tr><br/>
            <tr>
              <td>变异常数1：</td>
              <td><el-input clearable v-model="parameters.Pm1" :disabled="disabledStatus"></el-input></td>
            </tr><br/>
            <tr>
              <td>变异常数2：</td>
              <td><el-input clearable v-model="parameters.Pm2" :disabled="disabledStatus"></el-input></td>
            </tr><br/>
            <tr>
              <td>自变量离散精度eps：</td>
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:0,xlimr:1,yliml:-5,ylimr:5
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      if (this.parameters.Pc1>this.parameters.Pc2){
        this.$message.error('Pc1必须小于Pc2');
        return;
      }
      if (this.parameters.Pm1>this.parameters.Pm2){
        this.$message.error('Pm1必须小于Pm2');
        return;
      }
      this.$refs.funcImgArea.loading.status = true;
      this.$refs.funcImgArea.loading.text = "请稍等...."
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/ih/MMAdapGA',
        method: 'get',
        params: {
          ...
          Pc1: this.parameters.Pc1,
          Pc2: this.parameters.Pc2,
          Pm1: this.parameters.Pm1,
          Pm2: this.parameters.Pm2,
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='5*cos(10*pi*x)*exp(-2*x)'
      this.parameters.a=0
      this.parameters.b=1
      this.parameters.NP=80
      this.parameters.NG=500
      this.parameters.Pc1=0.6
      this.parameters.Pc2=0.9
      this.parameters.Pm1 = 0.02
      this.parameters.Pm2 = 0.05
...
<style>
.mmadapga-pc .dialog-container .el-dialog{
  height: 60%;
}/*

<template>
  <div class="adapga-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用自适应遗传算法求解一维无约束优化问题"/>
    </div>
...
export default {
  ...
  methods: {
    run(payload) {
      ...
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/ih/AdapGA',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='x^2-10*cos(2*pi*x)+10'
      this.parameters.a = 0
      this.parameters.b = 4
      this.parameters.NP=50
      this.parameters.NG=500
      this.parameters.Pc1=0.5
...
<style>
.adapga-pc .dialog-container .el-dialog{
  height: 60%;
}/*

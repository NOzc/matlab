<template>
  <div class="sboga-pc" style="height: 100vh;width: 100%;">
    <!--  顶部标题部分  -->
    <div style="height: 15%;padding: 10px;position: relative;background-color: #802c8c;">
      <p-c-title-area @run="run" @pause="pause" @displayDrawer="displayDrawer" @showDialog="showDialog" @resetAllSettings='resetAllSettings' title="用顺序选择遗传算法求解一维无约束优化问题"/>
    </div>
      ...
      <div id="workarea" style="height: 100%;width: 30%;position: absolute;left: 60px;">
        <p-c-params-area ref="paramsArea">
          <table>
            ...
            <tr>
              <td>最好个体的选择概率q：</td>
              <td><el-input clearable v-model="parameters.q" :disabled="disabledStatus"></el-input></td>
            </tr><br/>
            <tr>
              <td>杂交概率Pc：</td>
              <td><el-input clearable v-model="parameters.NG" :disabled="disabledStatus"></el-input></td>
...
export default {
  ...
  data() {
    return {
      ...
      xyStyle: {
        limit: {
          xliml:0,xlimr:4,yliml:-4,ylimr:2
        },
        label: {
  ...
  methods: {
    run(payload) {
      ...
      //向后端发起请求，获取图像
      axios({
        url: 'http://127.0.0.1:8080/matlab-server/workbench/ih/SBOGA',
        method: 'get',
        params: {
    ...
    resetAllSettings(){
      //参数设置
      this.parameters.func='x*sin(x)'
      this.parameters.a=0;
      this.parameters.b=4
      this.parameters.NP=50
      this.parameters.NG=500
      this.parameters.q=0.2
      this.parameters.Pc=0.9
      this.parameters.Pm=0.05
      ...
      this.xyStyle.limit.yliml=-4
      this.xyStyle.limit.ylimr=2
...
<style>
.sboga-pc .dialog-container .el-dialog{
  height: 60%;
}/*
